...
{
	echo "<div id='div1'>";
	echo "<form action='cotiza.php?accion=ListadoDeCuotas' name='form1' method='post' class='form-inline'>";
	echo '<fieldset><legend>Informaci&oacute;n Para Cotizaciones</legend>';
...
//			  console.log("New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')");
		...
		<label for="montocotizacion" class="sr-only control-label">Monto de Cotizacion a aplicar</label>
...
//			exit;
		...
		{
			echo "<div id='div1'>";
			// echo "<form action='cotiza.php?accion=Abonar' name='form1' method='post' onsubmit='return realizo_abono(form1)'>";
			...
			$sql="select numero, cedula, concat(trim(ape_tit), ', ', trim(nom_tit)) as nombre, ".$montocotizacion." AS cotizacion, cuenta from titulares where (ucase(status) = :statusA) or (ucase(status)=:statusJ) order by numero" ;// cedula";
			$maximo=20;
...
//			echo $sql_200;
			...
			mensaje(['tipo'=>'success','titulo'=>'Informaci&oacute;n preparada','texto'=>'<h2>Se ha generado la prenomina de las cotizaciones, puede proceder a la carga de servicios y la generacion de la nomina']);

			// probamdo enviar arreglos por url
			$fuente='cotizacionesxcobrar';
			$condicion="fecha='".$fechadescuento."' and concepto=39 and titulares.cedula=cotizacionesxcobrar.cedula";
			$arreglo=array(
				...
				"sentencia"=>"select *, concat(trim(ape_tit),', ',trim(nom_tit)) as nombre from titulares,". $fuente." where (concepto = :concepto) AND ".$condicion,
				"agrupar"=>"select concepto from ". $fuente." where fecha='".$fechadescuento."' and concepto=39 group by concepto",
				);
...
*/
	echo "<form action='cotiza.php?accion=Asiento' name='form1' method='post' onsubmit='return realiza_asiento_montepio(form1)'>";
	$fechadescuento=$_POST['fechadescuento'];
	echo '<input type="hidden" name="fechadescuento" value = "'.$fechadescuento.'">';
	echo '<input type="hidden" name="nombre_archivo" value = "'.$nombre_archivo.'"/>';
...
function revisar($r200, $fechadescuento, $micedula, $ip, $gestor, $db_con, $montocotizacion, $momento, $referencia)
{
	...
	try
	{
		...
		////////////
		$posiciond=1;
		$lacolumnapres='colref'.$posiciond;
		$lacolumnanro='colnro'.$posiciond;
		$elnumero=$referencia;
		$nrocuenta='';
		$sql="insert into detalle_dcto (fecha, cedula, codigo, nombre, colref1, colpre1, ip, proceso) values (:fecha, :cedula, :codigo, :nombre, :concepto, :lacuota, :ip_registro, :proceso)";
		// ".$lacolumnapres." = ".$lacolumnapres."+' :lacuota', ".$lacolumnanro." = ':elnumero' where (cedula =:cedula)" ;
		$resdc=$db_con->prepare($sql);
		$resdc->execute(array(
			":fecha"=>$fechadescuento,
			":cedula"=>$micedula,
			":codigo"=>substr($r200['numero'],0,6),
			":nombre"=>$r200['nombre'] . $r200['nom_tit'],
...
//		echo $sql.$fechadescuento;
		...
		$stmt->execute();
		$sql="delete from detalle_dcto where fecha = :fechadescuento";
//		echo $sql.$fechadescuento;
		$stmt=$db_con->prepare($sql);
		$stmt->bindParam(":fechadescuento",$fechadescuento);
		$stmt->execute();
